...
def generate_interface(service_name, methods):
    ...
    for method_name, param_types in methods.items():
        params_str = ", ".join(f"{param_type.capitalize() if param_type == 'string' else param_type} param{i+1}" for i, param_type in enumerate(param_types))
...
#         file_name = (path+f"{service_name}.java")
        file_name = f"{service_name}.java"
